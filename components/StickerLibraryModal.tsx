...
export const StickerLibraryModal: React.FC<StickerLibraryModalProps> = ({ isOpen, onClose, onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200 overflow-hidden">
        {/* Header */}
        ...
        <div className="px-6 py-4 flex items-center gap-6 border-b border-slate-50">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#3B5BFF]/20 focus:bg-white transition-all"
            />
          </div>
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col">
                      ...
                      <div className="flex-1 p-3 pb-0">
                        <div className={`w-full h-full rounded-xl flex items-center justify-center transition-all duration-300 ${tempSelection?.id === sticker.id ? 'bg-[#3B5BFF]/10' : sticker.color}`}>
                    ...
                    {tempSelection?.id === sticker.id && (
                      <div className="absolute top-3 left-3 z-10">
                        <div className="w-5 h-5 bg-[#3B5BFF] rounded-full flex items-center justify-center text-white shadow-md animate-in zoom-in-50 duration-200">
        ...
        <div className="p-6 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50/30">
          ...
          <button 
            ...
            className={`px-10 py-2.5 rounded-xl font-bold transition-all shadow-lg active:scale-95 ${tempSelection ? 'bg-[#3B5BFF] text-white shadow-[#3B5BFF]/20 hover:bg-[#3B5BFF]/90' : 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none'}`}
          >
            确认
          </button>
        </div>
